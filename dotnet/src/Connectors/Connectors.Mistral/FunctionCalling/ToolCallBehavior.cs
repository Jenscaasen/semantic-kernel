...
public abstract class ToolCallBehavior
{
    ...
    // the MistralPromptExecutionSettings has a single ToolCallBehavior property, but we could
    ...
    // the Kernel, KernelArguments, etc., and dynamically contribute tools to the MistralPromptExecutionSettings.
    ...
    /// <returns>
    /// The <see cref="ToolCallBehavior"/> that may be set into <see cref="MistralPromptExecutionSettings.ToolCallBehavior"/>
    /// to indicate that the specified functions should be made available to the model.
    /// </returns>
    public static ToolCallBehavior EnableFunctions(IEnumerable<MistralFunction> functions, bool autoInvoke = false)
    {
    ...
    /// </returns>
    public static ToolCallBehavior RequireFunction(MistralFunction function, bool autoInvoke = false)
    {
    ...
    /// <param name="options">The destination <see cref="MistralPromptExecutionSettings"/> to configure.</param>
    internal abstract void ConfigureOptions(Kernel? kernel, MistralPromptExecutionSettings options);
    ...
    internal sealed class KernelFunctions : ToolCallBehavior
    {
        ...
        internal override void ConfigureOptions(Kernel? kernel, MistralPromptExecutionSettings options)
        {
    ...
    {
        private readonly MistralFunction[] _mistralFunctions;
        private readonly FunctionDefinition[] _functions;

        public EnabledFunctions(IEnumerable<MistralFunction> functions, bool autoInvoke) : base(autoInvoke)
        {
            this._mistralFunctions = functions.ToArray();

            var defs = new FunctionDefinition[this._mistralFunctions.Length];
            for (int i = 0; i < defs.Length; i++)
            {
                defs[i] = this._mistralFunctions[i].ToFunctionDefinition();
            }
        ...
        internal override void ConfigureOptions(Kernel? kernel, MistralPromptExecutionSettings options)
        {
            MistralFunction[] MistralFunctions = this._mistralFunctions;
            FunctionDefinition[] functions = this._functions;
            Debug.Assert(MistralFunctions.Length == functions.Length);
    ...
    {
        private readonly FunctionDefinition _tool;
        private readonly ChatCompletionsToolChoice _choice;

        public RequiredFunction(MistralFunction function, bool autoInvoke) : base(autoInvoke)
        {
            this._tool = function.ToFunctionDefinition();
            this._choice = ChatCompletionsToolChoice.Any;
        }
